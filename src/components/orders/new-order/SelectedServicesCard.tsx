...
export default function SelectedServicesCard({
    ...
    return services.map((service: IOrderService, index: number) => (
        <div
            key={service._id}
            ...
            <div className="grid pl-4 gap-2">
                {service.complexity && (
                    <span className="text-sm text-muted-foreground mt-1 sm:mt-0">
                        <strong>Complexity:</strong> {service.complexity.name}
                    </span>
                )}

                {(service.options ?? []).length > 0 && (
                    <div className="flex flex-wrap gap-2 items-center text-sm">
                        <strong className="text-muted-foreground">Options:</strong>
                        ...
                        <ul className="pl-4 list-decimal">
                            {service.types?.map((type) => (
                                <li key={type._id}>
                                    ...
                                    {type.complexity && (
                                        <span> → {type.complexity.name}</span>
                                    )}
                                </li>
                            ))}
                        </ul>
